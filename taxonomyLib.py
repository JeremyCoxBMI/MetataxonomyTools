...
LINNAEUS_TAXONOMY = copy.deepcopy(LINNAEUS_TAXONOMY_REVERSE)
LINNAEUS_TAXONOMY.reverse()
...
def getSpeciesIDGenusID(taxonID, nodes):
    ...
    while (currID in nodes):
        ...
        z += 1
        if z >= MAX_ITERATIONS:
            print >> sys.stderr, "taxonomyLib::findSpeciesIDGenusID coudl not find taxon\t" + str(
                taxonID) + "\tresults spec\t" + str(specID) + "\tgenus\t" + str(genusID)
            break
        currID = nodes[currID][0]
...
#LINNAEUS_TAXONOMY_REVERSE = ["species","genus", "family", "order", "class","phylum","kingdom","superkingdom"]
    ...
    while True:
        if k in nodes:
            ...
            return str(k) + "_has_no_parent_node"
