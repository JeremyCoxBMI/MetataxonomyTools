...
all_total=0
total=0
for taxonID in speciesToFirstTaxonLength:
    firstDict = speciesToFirstTaxonLength[taxonID]
    maxLength = (0,"fakeName")
    for f in firstDict:
      if maxLength[0] < firstDict[f]:
        maxLength = (firstDict[f],f)
      outF.write(taxonID+"\t"+f+"\t"+str(firstDict[f])+"\n")
      all_total += firstDict[f]
    outF2.write(taxonID+"\t"+maxLength[1]+"\t"+str(maxLength[0])+"\n")
    total += maxLength[0]
